...
CreatePixel(pixel){
client.createPixel("walk",pixel).then(res => console.log(res.data))
...
client.getGraphs().then(res => console.log(res.data))
  ...
  handleDeletePixel =(deleteDate)=>{
    this.deletePixel('walk',deleteDate)
  ...
  render() {
    return (
      <div className="content">
          ...
          <br/>
        <WalkForm handleDate = {this.handleDate} handleCheckBox = {this.handleCheckBox} UpdateHabit = {this.UpdateHabit}/>
        {this.state.name &&  <Button className="btn-fill" color="primary" type="submit" onClick={this.getCreateUserApi(this.state.name.split(" ").join(''),'12345678')} >create Walk Graph</Button>}
 ...
 {this.state.name && 
<iframe id='pixelaFram' src={`https://pixe.la/v1/users/${this.state.name.split(" ").join('')}/graphs/walk.html`} title="description" style={{"width" :'700px', 'height':'450px'}}></iframe>}
